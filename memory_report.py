...
class MemoryReport:
    ...
    def hourly_line_chart(self, title, df, hostname, clientname):
        ...
        if Path(savepath).exists() or FORCE_GENERATE_CHARTS:
            return
        plt.clf()
        ...
        for anhour in client_hours.collect():
            ...
            df_for_hour = df.filter(hour('HR_Time') == lit(anhour[0])).groupBy().sum('AVG_Memory_Usage_Percentage')
            temp.append(df_for_hour.toPandas()["sum(AVG_Memory_Usage_Percentage)"].values.tolist())
    ...
    def weekly_bar_chart(self, title, df, hostname, clientname):
        ...
        for aday in client_days.collect():
            x.append(aday[0])
            #her saat icin avg processor time columni topla
            df_for_day = df.filter(dayofweek('HR_Time') == lit(aday[0])).groupBy().sum('AVG_Memory_Usage_Percentage')
            temp.append(df_for_day.toPandas()["sum(AVG_Memory_Usage_Percentage)"].values.tolist())
    ...
    def monthly_line_chart(self, title, df, hostname, clientname):
        ...
        for aday in client_days.collect():
            ...
            #her saat icin avg processor time columni topla
            df_for_day = df.filter(dayofmonth('HR_Time') == lit(aday[0])).groupBy().sum('AVG_Memory_Usage_Percentage')
            temp.append(df_for_day.toPandas()["sum(AVG_Memory_Usage_Percentage)"].values.tolist())
    ...
    def all_hosts_pie_chart(self, pdf):
        ...
        d2_sizes = []
        for host in self.client.hostnames:
            df = self.client.hosts_dataframes[host].select('AVG_Memory_Usage_Percentage').groupBy().sum()
...
if __name__ == "__main__":
    ...
    # load_to_database("./data/memory.xlsx", "basanmemory")
