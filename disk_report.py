...
class DiskReport:
    ...
    def hourly_line_chart_for_column(self, title, df, hostname, clientname, disk, columnname, columnname2):
        ...
        for anhour in client_hours.collect():
            ...
            df_for_hour = df.filter(hour('HR_Time') == lit(anhour[0])).groupBy().sum(f'{columnname}')
            temp.append(df_for_hour.toPandas()[f"sum({columnname})"].values.tolist())

            df_for_hour2 = df.filter(hour('HR_Time') == lit(anhour[0])).groupBy().sum(f'{columnname2}')
            temp2.append(df_for_hour2.toPandas()[f"sum({columnname2})"].values.tolist())
    ...
    def all_hosts_pie_chart(self, pdf):
        ...
        d2_sizes = []
        for host in self.client.hostnames:
            df = self.client.hosts_dataframes_w[host].select('AVG_Disk_Writes/Sec').groupBy().sum()
            d2_sizes.append(df.toPandas()["sum(AVG_Disk_Writes/Sec)"].values.tolist())
...
if __name__ == "__main__":
    ...
    print("hellomello")
    #load_to_database("./data/disk.xlsx", "basandisk")
    print("kello")
